...
const ViewProposalStatus = () => {
  ...
  useEffect(() => {
    const fetchProposals = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/proposals/status/${selectedStatus}`);
  ...
  const handleTrackClick = async (submissionId) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/proposals/${submissionId}/status`);
      setSelectedStatusList(response.data);
      setIsModalOpen(true); 
    } catch (error) {
      console.log("Error fetching proposal status history:", error.message);
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-1 space-y-6">
        <h1 className="custom-header text-2xl font-bold pt-10 mb-4">TRACK YOUR PROPOSAL STATUS!</h1>
        ...
        {proposals.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-center text-gray-500 mt-8">
            ...
            <h2 className="text-lg text-blue-800 font-bold">You haven't received any proposals yet</h2>
              ...
              >
                <div className="event-group flex-1 flex flex-row gap-1">
                  <h2 className="event-name text-lg font-bold">
                    {proposal.submission_status?.event_name || "Event Name Unavailable"}
                  </h2>
                  <p className="event-desc text-gray-700">
                    {proposal.submission_status?.proposal_name || "Proposal Name Unavailable"}
                  </p>
                  <p className="event-date text-gray-700">
                    {proposal.submission_status?.event_date
                      ? new Date(proposal.submission_status.event_date).toUTCString().substring(0, 16)
                      : "Unknown Date"}
                  </p>
                   <div className="flex items-center gap-3">
                  <button
                    onClick={() =>
                      navigate(`/view-proposal-detail/${proposal.submission_id}`)
                    }
                  ...
                  <button
                    onClick={() => handleTrackClick(proposal.submission_id)}
